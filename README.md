...
- `TOKEN`：访问口令。**设置后启用认证**：浏览器访问会先跳转到 `/login` 登录页，输入口令后获得 `HttpOnly` cookie（30 天有效）；也兼容 URL `?token=<TOKEN>` 或 `X-Token` 请求头（供脚本使用）。`?token=` 登录成功后会自动换成 cookie，前端会把 token 从地址栏移除。访问 `/logout` 可登出。口令不会传给子 shell（`env` 中不可见）。
